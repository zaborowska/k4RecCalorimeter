...
f = root.TFile("ODD_cells_endcap.root")
myTree = f.Get("cells")
columns = ["x", "y", "z", "xA","yA","zA", "xB", "yB", "zB", "xC", "yC", "zC", "xD", "yD", "zD","phi","dphi","R","dR","dz"]
df = pd.read_csv("ECalEndcap_endcap_0_cellPositions.txt", skipinitialspace=True, usecols=columns)
...
errorboxes_xyAC = []
#for x, y, xa, ya, xc, yc in zip(df.x, df.y, df.xA, df.yA, df.xB, df.yB):
...
#pc_xyCart = PatchCollection(errorboxes_xyCart, facecolor='b', alpha=0.2, edgecolor='black', linestyle="-", linewidth=1)
#pc_yxCart = PatchCollection(errorboxes_yxCart, facecolor='b', alpha=0.2, edgecolor='black', linestyle="-", linewidth=1)
...
#for x, y, xa, ya, xc, yc in zip(df.z, df.y, df.zD, df.yD, df.zC, df.yC):
#        errorboxes_zyCart.append(Polygon(((x-xc+xa,y-yc+ya),(x+xa+xc,y+ya+yc), (x+xc-xa,y+yc-ya), (x-xa-xc,y-ya-yc))))
#for x, y, xa, ya, xc, yc in zip(df.z, df.x, df.zD, df.xD, df.zC, df.xC):
...
#pc_zxCart = PatchCollection(errorboxes_zxCart, facecolor='b', alpha=0.2, edgecolor='black')
